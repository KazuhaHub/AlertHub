...
type Server struct {
	...
	// proxyWarned keeps the undeclared-proxy warning to one line per process.
	// It is a nudge for whoever reads the log, not a counter — the same request
	// would otherwise emit it on every login attempt behind that proxy.
